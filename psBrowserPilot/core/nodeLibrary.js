  ...
  return constants
    .map((constant) => {
      ...
      if (type === 'CheckBox' || type === 'RadioButton') {
        value = normalizeBooleanConstant(sourceValue);
      } else {
        const trimmed = typeof sourceValue === 'string' ? sourceValue.trim() : '';
        value = trimmed || DEFAULT_CONSTANT_PLACEHOLDER;
      }
      return {
        key,
        type,
      ...
      return {
        ...
        controls: normalized.constants.map((constant) => {
          ...
          return {
            ...
            controlKind,
            type: controlKind === 'TextBox' || controlKind === 'Reference' ? 'text' : controlKind,
            placeholder:
              controlKind === 'TextBox' || controlKind === 'Reference'
                ? constant.value || DEFAULT_CONSTANT_PLACEHOLDER
                : undefined,
            default:
              controlKind === 'CheckBox' || controlKind === 'RadioButton'
                ? normalizeBooleanConstant(constant.value)
                : constant.value,
          };
        }),
