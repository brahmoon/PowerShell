export function exportScript(script) {
  ...
  dialog.querySelector('#download-script').addEventListener('click', async () => {
    const electronAPI = typeof window !== 'undefined' ? window.electronAPI : null;
    if (electronAPI?.saveScript) {
      try {
        const result = await electronAPI.saveScript({
          content: script,
          suggestedName: 'flow.ps1',
        });
        if (result?.canceled) {
          return;
        }
        if (feedback) {
          feedback.textContent = `${result?.fileName || 'flow.ps1'} として保存しました。`;
          feedback.style.display = 'block';
        }
        return;
      } catch (error) {
