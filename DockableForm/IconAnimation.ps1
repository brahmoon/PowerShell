...
function EndDrag {
    ...
    $buttonValue = $button -as [System.Windows.Forms.MouseButtons]
    if ($null -eq $buttonValue) {
        $buttonValue = [System.Windows.Forms.MouseButtons]::None
    }

    if ($buttonValue -eq [System.Windows.Forms.MouseButtons]::Left -and -not $script:isAnimating) {
